import { type MaskInputOptions, maskInputValue, Mirror, getInputType, toLowerCase } from '@posthog/rrweb-snapshot'
...
import { IncrementalSource, MouseInteractions, PointerTypes, MediaInteractions } from '@posthog/rrweb-types'
import type {
    mutationCallBack,
    mousemoveCallBack,
    mousePosition,
...
export function initMutationObserver(
    ...
    const observer = new (mutationObserverCtor() as new (callback: MutationCallback) => MutationObserver)(
...
export function initScrollObserver({
    ...
    blockSelector,
    sampling,
}: Pick<observerParam, 'scrollCb' | 'doc' | 'mirror' | 'blockClass' | 'blockSelector' | 'sampling'>): listenerHandler {
    const updatePosition = callbackWrapper(
        throttle<UIEvent>(
            callbackWrapper((evt) => {
                const target = getEventTarget(evt)
...
function initViewportResizeObserver({ viewportResizeCb }: observerParam, { win }: { win: IWindow }): listenerHandler {
    let lastH = -1
    let lastW = -1
...
function initStyleSheetObserver(
    ...
    try {
        win.CSSStyleSheet.prototype.insertRule = new Proxy(insertRule, {
            apply: callbackWrapper(
                (target: typeof insertRule, thisArg: CSSStyleSheet, argumentsList: [string, number | undefined]) => {
    ...
    try {
        win.CSSStyleSheet.prototype.deleteRule = new Proxy(deleteRule, {
            apply: callbackWrapper((target: typeof deleteRule, thisArg: CSSStyleSheet, argumentsList: [number]) => {
    ...
    if (win.CSSStyleSheet.prototype.replace) {
        ...
        win.CSSStyleSheet.prototype.replace = new Proxy(replace, {
            apply: callbackWrapper((target: typeof replace, thisArg: CSSStyleSheet, argumentsList: [string]) => {
    ...
    if (win.CSSStyleSheet.prototype.replaceSync) {
        ...
        win.CSSStyleSheet.prototype.replaceSync = new Proxy(replaceSync, {
            apply: callbackWrapper((target: typeof replaceSync, thisArg: CSSStyleSheet, argumentsList: [string]) => {
                const [text] = argumentsList
    ...
    Object.entries(supportedNestedCSSRuleTypes).forEach(([typeKey, type]) => {
        ...
        try {
            type.prototype.insertRule = new Proxy(unmodifiedFunctions[typeKey].insertRule, {
                apply: callbackWrapper(
                    (target: typeof insertRule, thisArg: CSSRule, argumentsList: [string, number | undefined]) => {
                        const [rule, index] = argumentsList
            ...
            type.prototype.deleteRule = new Proxy(unmodifiedFunctions[typeKey].deleteRule, {
                apply: callbackWrapper((target: typeof deleteRule, thisArg: CSSRule, argumentsList: [number]) => {
                    const [index] = argumentsList
...
function initStyleDeclarationObserver(
    ...
    try {
        win.CSSStyleDeclaration.prototype.setProperty = new Proxy(setProperty, {
            apply: callbackWrapper(
                (target: typeof setProperty, thisArg: CSSStyleDeclaration, argumentsList: [string, string, string]) => {
...
function initCustomElementObserver({ doc, customElementCb }: observerParam): listenerHandler {
    ...
    const restoreHandler = patch(
        ...
        'define',
        function (
            original: (name: string, constructor: CustomElementConstructor, options?: ElementDefinitionOptions) => void
        ) {
            return function (name: string, constructor: CustomElementConstructor, options?: ElementDefinitionOptions) {
                try {
                    customElementCb({
                        define: {
                            name,
